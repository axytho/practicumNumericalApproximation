...
%   where y = spline(t)
    ...
    for i = 1:N
       ...
       while(x_i < n+1) && (~(t(i) < x(x_i)) && (t(i)>x(x_i-1)))
           % Note: the time complexity of this search algortihm is O(n), this
           % could be improved substantially with more complex search algoritms (eg. binairy search O(log(n))),
           % to shorten the runtime of this function when a lot of abscisses are given.
           x_i = x_i + 1;
           funcNum = funcNum + 1;
       end           
       if x_i == n+1
