...
%                                       ]
%
%this has a result a single value which is equal to the 
%v(i, j) = sum(sum(C(index1, index2) * x(i)^index1 * y(j)^index2)) 
%and we go over index1 and index2 for all C values 
%(C = p in the documentation)	
